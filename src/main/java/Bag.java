...
public abstract class Bag {
    ...
    public boolean addItem(String item) {
        ...
        if(this.numberOfContents < this.capacity) {
            ...
            String[] updated = new String[numberOfContents];
            if(this.numberOfContents == 1) {
                updated[0] = item;
            }
            else {
                for(int x = 0; x < this.numberOfContents - 1; ++x) {
                    updated[x] = this.contents[x];
                }
                updated[this.numberOfContents - 1] = item;
            }
    ...
    public String popItem() {
        ...
        if(this.numberOfContents > 0) {
            this.numberOfContents--;
            r = this.contents[this.numberOfContents];

            String[] updated = new String[this.numberOfContents];
            for(int x = 0; x < this.numberOfContents; ++x) {
                updated[x] = this.contents[x];
            }
            this.contents = updated;
        }
